...
function buildNaturalSelfieAction (cmd: string): string {
  ...
  if (looksLikeSelfieGroupPhotoRequest(cmd)) {
    return [
      ...
      '如果用户艾特了多个对象，或提供了多张参考图，应尽量把这些对象分别作为不同同框人物出现在合影里。',
      '如果同框对象来自动漫、游戏、二次元头像、插画、卡通图或角色头像，必须将其真人化为真实人类同框对象。',
      '动漫 / 游戏角色真人化时，应保留其标志性发型、发色、服装配色、角色气质和辨识特征，但面部、皮肤、身体、姿势都应是自然真实人类。',
      '不要把动漫 / 游戏 / 二次元角色生成玩偶、手办、毛绒玩具、抱枕、贴纸、立牌、Q版小人或道具。',
      '所有同框对象都应该是画面里的真实人物，而不是摆在旁边的物品。',
      '画面应像真实拍下的一张自然多人合影，统一光线、统一透视、统一风格，人物站位和姿势协调。',
