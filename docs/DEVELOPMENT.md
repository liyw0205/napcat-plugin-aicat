...
- `verify:stage12` 是无外部依赖的纯函数单元回归，覆盖配置归一化、消息安全、AI 权限、图片工具和代理 URL 处理。
- `verify` 串联配置验证、源码类型检查、代理回归、stage4 运行回归、stage6 Web smoke、stage7 Web DOM 回归、stage8 生命周期 smoke、stage9 Provider contract smoke、stage10 真实浏览器预检、stage11 真实 Provider 预检、stage12 单元回归和构建。
...
5. `verify:stage12` 已建立轻量单元回归入口，但尚未引入 lint 或完整测试框架。
